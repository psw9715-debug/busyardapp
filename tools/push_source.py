...
def parse(path):
    ...
    for c0 in FIRST_COLS:
        for r in ROWS:
            ...
            when = grid[r - 1][c0]
            if isinstance(when, (datetime.time, datetime.datetime)):
                cars[plate] = {"rest": False, "out": when.strftime("%H:%M")}
            else:
                cars[plate] = {"rest": True, "out": None}
    if not cars:
...
def main():
    ...
    rest = sum(1 for v in cars.values() if v["rest"])
    print(f"{out} — {len(cars)}대 (휴차 {rest})")
    print(f"  원본 {path}")
